...
func (db *DB) CreateContainer(c *Container) error {
	...
	query := `
		INSERT INTO containers (name, service_type, image, icon_url, ports, volumes, environment, compose_path, enabled, docker_id, network_mode, privileged)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`

	result, err := db.Exec(query, c.Name, c.ServiceType, c.Image, c.IconURL, string(ports), string(volumes), string(environment), c.ComposePath, c.Enabled, c.DockerID, c.NetworkMode, c.Privileged)
	if err != nil {
...
func (db *DB) GetContainer(id int64) (*Container, error) {
	query := `SELECT id, name, service_type, image, COALESCE(icon_url, '') as icon_url, ports, volumes, environment, compose_path, enabled, docker_id, COALESCE(network_mode, '') as network_mode, COALESCE(privileged, 0) as privileged, created_at, updated_at FROM containers WHERE id = ?`

	c := &Container{}
	var ports, volumes, environment string

	err := db.QueryRow(query, id).Scan(&c.ID, &c.Name, &c.ServiceType, &c.Image, &c.IconURL, &ports, &volumes, &environment, &c.ComposePath, &c.Enabled, &c.DockerID, &c.NetworkMode, &c.Privileged, &c.CreatedAt, &c.UpdatedAt)
...
func (db *DB) ListContainers() ([]*Container, error) {
	query := `SELECT id, name, service_type, image, COALESCE(icon_url, '') as icon_url, ports, volumes, environment, compose_path, enabled, docker_id, COALESCE(network_mode, '') as network_mode, COALESCE(privileged, 0) as privileged, created_at, updated_at FROM containers ORDER BY created_at DESC`
	...
	for rows.Next() {
		...
		err := rows.Scan(&c.ID, &c.Name, &c.ServiceType, &c.Image, &c.IconURL, &ports, &volumes, &environment, &c.ComposePath, &c.Enabled, &c.DockerID, &c.NetworkMode, &c.Privileged, &c.CreatedAt, &c.UpdatedAt)
		if err != nil {
...
func (db *DB) UpdateContainer(c *Container) error {
	...
	query := `
		UPDATE containers
		SET name = ?, service_type = ?, image = ?, icon_url = ?, ports = ?, volumes = ?, environment = ?, compose_path = ?, enabled = ?, docker_id = ?, network_mode = ?, privileged = ?, updated_at = CURRENT_TIMESTAMP
		WHERE id = ?
	`

	_, err := db.Exec(query, c.Name, c.ServiceType, c.Image, c.IconURL, string(ports), string(volumes), string(environment), c.ComposePath, c.Enabled, c.DockerID, c.NetworkMode, c.Privileged, c.ID)
	if err != nil {
